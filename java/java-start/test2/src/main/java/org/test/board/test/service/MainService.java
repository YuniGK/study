...
public class MainService {
    ...
    public City getSearchKeyWordValue(String keyWord, String value) throws Exception {
        ...
        for (JsonNode node : rootNode) {
            for (JsonNode nodeList : node){
                if (nodeList.get(keyWord).textValue().equals(value)){
                    ...
                    log.info("for key={} value={}, iata={}, icao={}"
                            , keyWord, value, nodeList.path("iata").asText(), nodeList.path("icao").asText());

                    resultCity = City.of(
                            "icao"//nodeList.path("icao").asText()
        ...
        log.info("key={}, value={}, resultCity = {}", keyWord, value, resultCity);
